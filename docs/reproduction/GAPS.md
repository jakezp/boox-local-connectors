...
## Current Reader extension

The Drive companion now includes books, reading data and native Library Settings
on NeoReader38701. Real Drive fresh-book restore, annotation/handwriting readback,
three interruption checkpoints and explicit conflict selection passed on one
BOOX. Current suites:66 Android and140 host checks, with one optional host skip.
See the [current validation](../VALIDATION.md) and [Reader guide](../READER-DRIVE.md).

A physical second BOOX, large/long-running libraries, shelf synchronization and
book deletion propagation remain outside those acceptance results. The optional
Mac notebook client does not consume the separate Reader protocol namespace.
